"""
Agent 类封装 - 第七课升级版：记忆系统
在第六课（智能体循环）基础上，
新增 AgentMemory 和 run_with_memory() 方法。
...
请返回 JSON："""
    ...
    def run_with_memory(self, user_input: str) -> Optional[dict]:
        ...
        流程：
        1. 从记忆中检索所有存储的事实
        2. 将记忆拼入 Prompt，让 AI 看到
        3. AI 根据记忆和用户输入生成回复
        4. 如果 AI 决定存储新信息，自动保存到记忆

        Args:
            user_input: 用户输入

        Returns:
            包含 reply 和 save_to_memory 的字典，失败则返回 None
        """
...
if __name__ == "__main__":
    ...
    # 对话2：问它记不记得
    print("--- 对话 2 ---")
    r2 = agent.run_with_memory("你还记得我的名字和职业吗？")
    if r2:
