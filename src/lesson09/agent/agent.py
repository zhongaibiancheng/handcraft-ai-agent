"""
Agent 类封装 - 第九课升级版：原子动作系统
在第八课（规划系统）基础上，
新增 agent/planner.py 的 create_atomic_action() 和 Agent.create_atomic_action() 方法。
...
class Agent:
    ...
    新增功能（第九课）：
    - create_atomic_action()：将步骤转为带 schema 的原子动作
    - agent/planner.py：新增 create_atomic_action() 函数
...
请返回 JSON："""
    ...
    def create_atomic_action(self, step: str) -> Optional[dict]:
        ...
        Args:
            step: 计划中的一个步骤（例如，"撰写初稿"）

        Returns:
            带 "action" 和 "inputs" 的原子动作字典，失败则返回 None
...
if __name__ == "__main__":
    ...
    # 计划 → 原子动作
    print("\n\n--- 计划 → 原子动作 ---")
    plan = agent.create_plan("创建一个 Python 教程")
    if plan:
        actions = agent.convert_plan_to_atomic_actions(plan)
        print(f"\n共生成 {len(actions)} 个原子动作")
