"""
Agent 类封装 - 第十一课升级版：评测系统
在第十课（AoT 思维原子）基础上，
新增 agent/evals.py 的 AgentEval、黄金数据集机制
以及 Agent.run_evals()、Agent.run_quick_eval() 方法。
...
class Agent:
    ...
    新增功能（第十一课）：
    - run_evals()：运行全量评测（结构化输出 + 工具调用 + 记忆）
    - run_quick_eval()：运行快速评测（仅结构化输出）
    ...
    保留功能（第二课、第三课）：System Prompt、多轮对话、结构化输出
    """
...
请返回 JSON："""
    ...
    def show_info(self) -> None:
        ...
        if self.state.current_plan:
            print(f"  当前计划：{len(self.current_plan.get('steps', []))} 个步骤")
        if self.state.current_aot_graph:
            print(f"  当前 AoT 图：{len(self.current_aot_graph.get('nodes', []))} 个节点")
        print("=" * 55)
