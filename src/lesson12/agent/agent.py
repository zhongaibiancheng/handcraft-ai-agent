"""
Agent 类封装 - 第十二课升级版：遥测系统（运行时可观测性）
在第十一课（评测系统）基础上，
新增 agent/telemetry.py 的 Span、Metrics、Telemetry 类
以及 Agent 方法的遥测仪表化。

新增功能（第十二课）：
- Telemetry：结构化日志（JSONL）+ 追踪 ID + 指标累积
- 方法级仪表化：自动记录每个 LLM 调用、工具调用、记忆操作的耗时与结果
- show_telemetry()：打印遥测摘要
- analyze_trace()：按追踪 ID 调试特定交互
...
class AgentState:
    ...
    def __init__(self):
        ...
        self.current_plan: Optional[dict] = None          # 第八课新增
        self.current_aot_graph: Optional[dict] = None     # 第十课新增
...
class AgentMemory:
    ...
    def __init__(self, telemetry=None):
        ...
        self._telemetry = telemetry  # 第十二课：可选的遥测引用
    ...
    def add(self, fact: str) -> None:
        ...
        if self._telemetry:
            self._telemetry.log_memory_op("add", fact, duration)
    ...
    def get_all(self) -> list[str]:
        ...
        duration = (time.time() - start) * 1000
        if self._telemetry:
            self._telemetry.log_memory_op("get_all", None, duration)
        return result
    ...
    def search(self, keyword: str) -> list[str]:
        ...
        result = [m for m in self.memories if keyword in m.lower()]
        duration = (time.time() - start) * 1000
        if self._telemetry:
            self._telemetry.log_memory_op("search", keyword, duration)
        return result
    ...
    def remove(self, fact: str) -> bool:
        ...
        if fact in self.memories:
            ...
            duration = (time.time() - start) * 1000
            if self._telemetry:
                self._telemetry.log_memory_op("remove", fact, duration)
            return True
        duration = (time.time() - start) * 1000
        if self._telemetry:
            self._telemetry.log_memory_op("remove", fact, duration)
    ...
    def clear(self) -> None:
        ...
        print("🧹 所有记忆已清空")
        if self._telemetry:
            self._telemetry.log_memory_op("clear", None, duration)
...
class Agent:
    ...
    - show_telemetry()：打印遥测摘要
    - analyze_trace()：按追踪 ID 调试特定交互
    - 所有核心方法自动记录遥测数据（LLM 调用、工具调用、记忆操作、决策）

    保留功能（第十一课）：run_evals()、run_quick_eval()
    保留功能（第十课）：create_aot_plan()、execute_aot_plan()、print_aot_graph()
    保留功能（第九课）：create_atomic_action()、convert_plan_to_atomic_actions()
    保留功能（第八课）：create_plan()、execute_plan()
    保留功能（第七课）：AgentMemory
    保留功能（第六课）：AgentState、agent_step()、run_loop()
    保留功能（第五课）：register_tool()、request_tool()、execute_tool_call()
    保留功能（第四课）：decide()、decide_with_descriptions()、route()
    保留功能（第一、二、三课）：对话、System Prompt、结构化输出
    """
    ...
    def __init__(
        ...
        log_file: str = "agent_telemetry.jsonl",
    ):
        ...
        # 第七课：记忆系统（第十二课：传入 telemetry 引用）
        self.telemetry = None  # 先设为 None，初始化时传入
        self.memory = AgentMemory(telemetry=self.telemetry)

        # 第十二课：遥测系统
        from agent.telemetry import Telemetry
        self.telemetry = Telemetry(log_file=log_file)
        self.memory._telemetry = self.telemetry  # 建立引用

    # ==================== 辅助方法（第十二课新增） ====================

    def _record_llm_call(self, start_time: float, prompt_text: str,
    ...
    def generate(self, prompt: str, temperature: float = 0.0) -> str:
        ...
        self._record_llm_call(start, prompt, result)
        return result

    def generate_with_role(self, user_input: str) -> str:
        """使用 System Prompt 生成回复（第十二课：带遥测记录）"""
        self._ensure_trace()
        start = time.time()
        ...
        result = response.choices[0].message.content.strip()
        self._record_llm_call(start, user_input, result)
        return result

    def chat(self, user_input: str) -> str:
        """带历史记忆的多轮对话（第十二课：带遥测记录）"""
        self._ensure_trace()
        start = time.time()
        ...
        self._record_llm_call(start, user_input, reply)
    ...
    def chat_stream(self, user_input: str) -> Generator[str, None, None]:
        ...
        self._record_llm_call(start, user_input, full_reply)
        self.history.append({"role": "user", "content": user_input})
...
请返回 JSON："""
        ...
        for attempt in range(3):
            ...
            if parsed is not None:
                self._record_llm_call(start, prompt, text, success=True, retries=retries)
                return parsed

            retries = attempt + 1

        # 所有重试都失败
        self._record_llm_call(start, prompt, text, success=False, retries=retries,
                              error="Failed to parse JSON after 3 attempts")
        return None

    # ==================== 第四课方法 ====================

    def _log_decision_telemetry(self, start_time: float, choices: list[str],
                                decision: Optional[str]) -> None:
        """记录决策到遥测系统。"""
        if self.telemetry:
            duration = (time.time() - start_time) * 1000
            if decision:
                self.telemetry.log_decision(choices, decision, duration)
            else:
                self.telemetry.log_decision(choices, "unknown", duration,
                                            error="Failed to make decision")

    def decide(self, user_input: str, choices: list[str]) -> Optional[str]:
        """让 AI 从选项列表中选择一个动作（第十二课：带遥测记录）"""
        self._ensure_trace()
        start = time.time()
        ...
        for attempt in range(3):
            ...
            if parsed and "decision" in parsed:
                ...
                if decision in choices:
                    self._record_llm_call(start, user_prompt, text)
                    self._log_decision_telemetry(start, choices, decision)
                    return decision
                for choice in choices:
                    if decision.lower().replace("_", "").replace("-", "") == choice.lower().replace("_", "").replace("-", ""):
                        self._record_llm_call(start, user_prompt, text)
                        self._log_decision_telemetry(start, choices, choice)
                        return choice

        self._record_llm_call(start, user_prompt, text, success=False,
                              error="Failed to make decision after 3 attempts")
        self._log_decision_telemetry(start, choices, None)
        return None
...
请返回 JSON："""
        ...
        self._record_llm_call(start, user_prompt, text, success=False,
                              error="Failed to make decision with descriptions")
        self._log_decision_telemetry(start, choices, None)
        return None
...
请返回 JSON："""
            ...
            if parsed and "tool" in parsed and "arguments" in parsed:
                if parsed["tool"] in self.tools:
                    ...
                    if self.telemetry:
                        ...
                        self.telemetry.log_tool_request(
                            parsed["tool"], parsed.get("arguments", {}), dur
                        )
                    return parsed
                for tool_name in self.tools:
                    if parsed["tool"].lower().replace("_", "").replace("-", "") == tool_name.lower().replace("_", "").replace("-", ""):
                        ...
                        if self.telemetry:
                            dur = (time.time() - start) * 1000
                            self.telemetry.log_tool_request(
                                tool_name, parsed.get("arguments", {}), dur
                            )
                        return parsed
    ...
    def execute_tool_call(self, tool_call: dict) -> Any:
        ...
        if tool_name not in self.tools:
            ...
            if self.telemetry:
                self.telemetry.log_tool_execution(
                    tool_name, None, duration, success=False,
        ...
        try:
            ...
            if self.telemetry:
                self.telemetry.log_tool_execution(
                    tool_name, result, duration, success=True
                )
            return result
            ...
            if self.telemetry:
                self.telemetry.log_tool_execution(
                    tool_name, None, duration, success=False, error=str(e)
                )
    ...
    def agent_step(self, user_input: str) -> Optional[dict]:
        """执行智能体循环的一步：观察→决策→执行（第十二课：带遥测记录）"""
        self._ensure_trace()
...
请返回 JSON："""
            ...
            if parsed and "action" in parsed:
                ...
                if self.telemetry:
                    ...
                    self.telemetry.log_decision(
                        valid_actions, parsed["action"], dur
                    )
    ...
    def run_loop(self, user_input: str, max_steps: int = 5) -> list[dict]:
        """运行多步智能体循环（第十二课：带遥测追踪）"""
        self._ensure_trace()
        trace_id = self.telemetry.current_trace_id if self.telemetry else "?"
        self.state.reset()
        results = []
...
请返回 JSON："""
        ...
        for attempt in range(3):
            ...
            self._record_llm_call(llm_start, user_prompt, text)
    ...
    def execute_plan(self, plan: dict) -> list[dict]:
        ...
        注意：本方法执行的是模糊步骤（占位执行）。
        如需原子动作执行，请用 convert_plan_to_atomic_actions() + 自定义执行逻辑。
        """
    ...
    def create_atomic_action(self, step: str) -> Optional[dict]:
        """
        将计划步骤转换为原子动作（第九课核心方法）。

        委托给 agent/planner.py 的 create_atomic_action() 函数。
        """
        from agent.planner import create_atomic_action as action_fn

        atomic = action_fn(self, step)

        if atomic:
            action_name = atomic.get("action", "?")
            inputs = atomic.get("inputs", {})
            print(f"⚛️ 原子动作：{action_name}({inputs})")

        return atomic

    def convert_plan_to_atomic_actions(self, plan: dict) -> list[dict]:
        """
        将整个计划的所有步骤转换为原子动作（第九课辅助方法）。
        """
        if not plan or "steps" not in plan:
            print("❌ 无效的计划")
            return []
        ...
        print(f"\n⚡ 开始转换计划为原子动作（共 {total} 个步骤）\n")

        for i, step in enumerate(plan["steps"], 1):
            print(f"--- 转换步骤 {i}/{total} ---")
            print(f"   原始步骤：{step}")

            atomic = self.create_atomic_action(step)

            if atomic:
                atomic["original_step"] = step
                atomic["step_num"] = i
                atomic_actions.append(atomic)
                print(f"   ✅ 转换成功\n")
            else:
                print(f"   ⚠️ 转换失败，保留原始步骤\n")
        ...
        success_count = sum(1 for a in atomic_actions if a.get("converted", True))
        print(f"📊 转换完成：{success_count}/{total} 个步骤成功转为原子动作")

        return atomic_actions

    # ==================== 第十课新增方法 ====================

    def create_aot_plan(self, goal: str) -> Optional[dict]:
        """
        生成 AoT 执行图（第十课核心方法）。

        委托给 agent/planner.py 的 create_aot_graph() 函数。
        """
        from agent.planner import create_aot_graph

        graph = create_aot_graph(self, goal)

        if graph:
            self.state.current_aot_graph = graph
    ...
    def execute_aot_plan(self, graph: dict) -> list[dict]:
        ...
        委托给 agent/planner.py 的 execute_graph() 函数。
        """
    ...
    def print_aot_graph(self, graph: dict) -> None:
        """
        以树状格式打印 AoT 图的依赖关系（第十课辅助方法）。
        """
        ...
        total = len(nodes)
        parallel = sum(1 for n in nodes if not n.get("depends_on"))
        max_depth = self._calc_graph_depth(graph)
        print(f"\n  📊 节点数：{total} | 可并行：{parallel} | 最大深度：{max_depth}")

    def _calc_graph_depth(self, graph: dict) -> int:
        """计算图的最大深度（第十课内部方法）"""
        nodes = graph["nodes"]
        ...
        return max(depth(n["id"]) for n in nodes) if nodes else 0

    # ==================== 第十一课新增方法 ====================

    def run_evals(self) -> list:
        """
        运行全量评测（第十一课核心方法）。
        """
        ...
        evaluator = AgentEval(self)

        print(f"\n🧪 开始全量评测（{len(STRUCTURED_OUTPUT_GOLDEN) + len(TOOL_CALL_GOLDEN) + len(MEMORY_GOLDEN)} 个用例）\n")
    ...
    def run_quick_eval(self) -> list:
        """
        运行快速评测（第十一课辅助方法）。
        """
        from agent.evals import AgentEval, print_eval_report
        from evals.golden_datasets import STRUCTURED_OUTPUT_GOLDEN

        evaluator = AgentEval(self)

        print(f"\n⚡ 快速评测（{len(STRUCTURED_OUTPUT_GOLDEN)} 个用例）\n")

        results = [evaluator.test_structured_output(STRUCTURED_OUTPUT_GOLDEN)]
    ...
    def analyze_trace(self, trace_id: Optional[str] = None) -> None:
        """
        分析指定追踪的详细信息（第十二课新增）。

        Args:
            trace_id: 要调试的追踪 ID。默认为当前追踪。
        """
        tid = trace_id or (self.telemetry.current_trace_id if self.telemetry else None)

        if not tid:
            print("❌ 未指定追踪 ID，且当前没有活跃的追踪")
            return

        if self.telemetry:
            self.telemetry.print_trace_detail(tid)
        else:
            print("⚠️ 遥测系统未启用")
    ...
    def show_state(self) -> None:
        ...
        if state.current_aot_graph:
            print(f"  当前 AoT 图：{len(state.current_aot_graph.get('nodes', []))} 个节点")
    ...
    def show_info(self) -> None:
        ...
        print(f"  遥测日志：{self.telemetry.log_file if self.telemetry else '未启用'}")
        if self.state.current_plan:
            print(f"  当前计划：{len(self.state.current_plan.get('steps', []))} 个步骤")
        if self.state.current_aot_graph:
            print(f"  当前 AoT 图：{len(self.state.current_aot_graph.get('nodes', []))} 个节点")
        print("=" * 55)
...
if __name__ == "__main__":
    ...
    # 查看当前追踪详情
    if agent.telemetry and agent.telemetry.current_trace_id:
