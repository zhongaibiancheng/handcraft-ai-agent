"""
Agent 类封装 - 第十课升级版：AoT（思维原子）系统
在第九课（原子动作）基础上，
新增 agent/planner.py 的 create_aot_graph()、execute_graph()
以及 Agent.create_aot_plan()、Agent.execute_aot_plan() 方法。
...
class Agent:
    ...
    新增功能（第十课）：
    - create_aot_plan()：生成 AoT 依赖图
    - execute_aot_plan()：按依赖关系执行图
